...
func (gr *Reconciler) observe(observables ...resource.Observable) (*resource.ObjectBag, error) {
	...
	for _, obs := range observables {
		...
		if obs.Labels != nil {
			...
			matchingLabels := client.MatchingLabels(obs.Labels)
			matchingLabels.ApplyToList(listOptions)
			listOptions.Raw = &metav1.ListOptions{TypeMeta: obs.Type}
			err = gr.List(context.TODO(), obs.ObjList.(runtime.Object), listOptions)
			if err == nil {
